...
def log_measurement(device_id=0, command_str='', ts_min=0, ts_max=0, status_str=''):
...
def update_device_ids(ts_from, ts_to, device_id): # -> dict[str, str]:
    ...
    if amount_selected == amount_committed:
        ...
        log_measurement(device_id=device_id, command_str='UPDATE', ts_min=ts_from, ts_max=ts_to, status_str=response['status']):
    else:
